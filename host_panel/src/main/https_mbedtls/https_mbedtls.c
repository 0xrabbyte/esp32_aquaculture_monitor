...
static char * gen_request(const char * server, const char * url) {
    char * request = NULL;
...
void https_get_task(void *pvParameters)
{
    char buf[512];
    int ret, flags, len;
    char server[64]; strcpy(server, ((https_client_config_t *)pvParameters)->server);
    char port[6]; strcpy(port, ((https_client_config_t *)pvParameters)->port);
    char url[256]; strcpy(url, ((https_client_config_t *)pvParameters)->url);

    char * request = gen_request(server, url);
    ESP_LOGI(TAG, "Requesting URL: %s", request);
